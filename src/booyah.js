...
export class MenuEntity extends entity.ParallelEntity {
  ...
  setup(config) {
    ...
    this.pauseButton = new PIXI.Sprite(this.config.app.loader.resources["booyah/images/button-mainmenu.png"].texture);
...
export class LoadingScene extends entity.CompositeEntity {
  ...
  setup(config) {
    ...
    this.loadingCircle = new PIXI.Sprite(this.preloader.resources["booyah/images/loader-circle.png"].texture);
...
function loadB1() {
  ...
  const pixiLoaderResources = [].concat(
    ...
    _.map(booyahConfig.videoAssets, name => { return { 
        url: `video/${name}`,
        metadata: {
...
function loadB2() {
  return narration.loadScript("fr").then((script) => {
...
export function go(config = {}) {
  ...
  gameStateMachine = new entity.StateMachine(booyahConfig.states, booyahConfig.transitions, {
