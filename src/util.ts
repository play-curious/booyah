...
export async function loadSubtitles(fileName: string): Promise<narration.ParsedSubtitle[]> {
  ...
  while (match = subtitleRegex.exec(plainText)) {
    ...
    output.push({
      startsAt: +start_ms + +start_s * 1000 + +start_m * 1000 * 60 + +start_h * 1000 * 60 * 60,
      endsAt: +end_ms + +end_s * 1000 + +end_m * 1000 * 60 + +end_h * 1000 * 60 * 60,
      text
    })
  }
